...
class WebServer:
    def __init__(self, html_folder: str, homeassistant_url: str, homeassistant_token: str, menu: dict, debug: bool):
        ...
        if self.homeassistant_url.startswith("http://") or self.homeassistant_url.startswith("https://"):
            self.homeassistant_url = self.homeassistant_url.replace("http://", "ws://").replace("https://", "wss://")
    ...
    def abort(self, code, description=''):
        ...
        response = make_response(f"<html><head><meta http-equiv=\"refresh\" content=\"60\"></head><body><h1>Error {code}</h1><br>{description}</body></html>", code)
    ...
    def serve_file(self, filename):
        try:
            with WebsocketClient(self.homeassistant_url, self.homeassistant_token) as homeassistant_client:
                ...
                if request.method == 'POST':
                    ...
                    if service is not None:
                        ...
                        with homeassistant_client.listen_events("state_changed") as events:
                ...
                if not os.path.isfile(file_path) or not os.path.commonpath([file_path, self.html_folder]) == self.html_folder:
                    return self.abort(404)
                
                if filename.endswith(".html"):
                        entities = homeassistant_client.get_entities()
                        entities['weather_forecast'] = homeassistant_client.trigger_service_with_response("weather", "get_forecasts", entity_id='weather.home',  type='daily')['weather.home']["forecast"]
            ...
            return self.abort(500, description=f"Exception: {type(e).__name__}, Arguments: {e.args}")
...
def main():
    ...
    web_server = WebServer(config["general"]["html_template"], 
    ...
    _LOGGER.info(f'Initialization done! Server is running {config["server"]["bind_addr"]}:{config["server"]["bind_port"]} ...')
